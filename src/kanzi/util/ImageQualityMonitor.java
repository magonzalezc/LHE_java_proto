...
public final class ImageQualityMonitor
{
   ...
   //usa las mismas constantes que el plugin de imageJ, pues son 001 y 003
   private static final int C1 = 104;    // 16 * ((0.01 * 255) ^ 2)
   private static final int C2 = 9 * C1; // 16 * ((0.03 * 255) ^ 2)

   // gaussian: 32 * exp(-k*x*x);
   //el filtro parece de ancho 7
   public static final int[] DEFAULT_GAUSSIAN_KERNEL = new int [] { 2, 9, 23, 32, 23, 9 , 2 };
  
   //un filtro mio de ancho 11
   // public static final int[] DEFAULT_GAUSSIAN_KERNEL = new int [] { 1,2, 4,8,16,32,16,8,4,2,1 };
                                ...
                                int x, int y, int w, int h, ColorModelType type)
   {
      ...
      return ((102*psnr1024_chanY) + (13*psnr1024_chanU) + (13*pnsr1024_chanV)) >> 7;
   }
   ...
   // Color model must be YUV444 or YUV420.
   // return psnr * 1024 or INFINITE_PSNR (=0)
   public int computePSNR_HVS_M(int[] src, int[] dst, int channelIdx, ColorModelType type) 
   {
      return this.computePSNR_HVS_M(src, dst, 0, 0, this.width, this.height, channelIdx, type);
   }
                          ...
                          int x, int y, int w, int h, ColorModelType type, int ds)
   {
      ...
      //solo Y, nada de U ni V .MODIFICATION JOSE JAVIER
      //return (int) ((128*ssim1024_chan1))  >> 7;
      
      if (this.grey=true) {
    	  System.out.println("SSIM IN GREY MODE");
    	  return (int) ((128*ssim1024_chan1))  >> 7; 
      }
      else	{
    	  System.out.println("SSIM IN COLOR MODE");
    	  return (int) ((102*ssim1024_chan1) + (13*ssim1024_chan2) + (13*ssim1024_chan3)) >> 7;
      }
   ...
   private static void computeBlockSSIM(Context ctx)
   {
     ...
     for (int y=yMin; y<=yMax; y+=inc)
     {
         ...
         for (int x=xMin; x<=xMax; x+=inc)
         {
            ...
            // System.out.println(xVal+"  ,"+yVal);//yyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyyy
